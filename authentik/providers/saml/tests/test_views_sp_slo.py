...
class TestSPInitiatedSLOViews(TestCase):
    ...
    def test_redirect_view_handles_logout_response_with_relay_state(self):
        ...
        return_url = "https://idp.example.com/flow/return"
        encoded_relay_state = base64.urlsafe_b64encode(return_url.encode()).decode()
        ...
        # Should redirect to decoded relay state
    ...
    def test_redirect_view_handles_logout_response_no_relay_state_with_session(self):
        """Test that redirect view uses session fallback when no RelayState"""
        session_return_url = "https://idp.example.com/flow/session-return"
        ...
        # Should redirect to session stored URL
        self.assertEqual(response.status_code, 302)
        self.assertEqual(response.url, session_return_url)
    ...
    def test_post_view_handles_logout_response_no_relay_state_with_session(self):
        """Test that POST view uses session fallback when no RelayState"""
        session_return_url = "https://idp.example.com/flow/session-return"
    ...
    def test_relay_state_decoding_failure(self):
        """Test handling of invalid base64 RelayState"""
        # Create request with invalid base64 relay state that looks like a path
        request = self.factory.get(
            f"/slo/redirect/{self.application.slug}/",
            {
                "SAMLResponse": "dummy-response",
